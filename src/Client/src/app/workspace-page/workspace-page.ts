...
import { CategoryModel, PaginatedResponse, TaskDraft, TaskEditDraft, TaskModel } from '../api.models';
...
export class WorkspacePage implements OnInit {
  ...
  @Input({ required: true }) currentEmail = '';
  @Output() loggedOut = new EventEmitter<string | undefined>();
  ...
  private handleError(error: HttpErrorResponse): void {
    ...
    if (error.status === 401) {
      this.loggedOut.emit('Session expired. Log in again.');
      this.refreshView();
      return;
    }
